...
const schema = values =>
  yup.object().shape({
    ...
    date_of_birth: yup.date().required("Please enter a valid date"),
    ...
    start_date: yup.date().required('Please enter a starting date').default(() => {
      return new Date
    }),
    employable_type: yup.mixed().oneOf(['worker', 'manager'], 'Must select an employee type')
    });
